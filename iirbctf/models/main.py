...
def train_loop(opt, loss_weights, logger, trainset, testset, model, optimizer):
    ...
    while it < opt.num_iters:
        ...
        for data in tqdm(trainloader, desc="Training for epoch " + str(epoch)):
            ...
            # decay learning rate
            if (
                it >= opt.learning_rate_decay_frequency
                and it % opt.learning_rate_decay_frequency == 0
            ):
...
def main():
    ...
    # print("---" * 5)
    # # add 32 batch dimension to source_img_data, target_img_data
    # # torch.Size([3, 224, 224]) -> torch.Size([32, 3, 224, 224])
    # sample_data['source_img_data'] = torch.stack([sample_data['source_img_data'] for i in range(32)])
    # sample_data['target_img_data'] = torch.stack([sample_data['target_img_data'] for i in range(32)])
    # print(sample_data['source_img_data'].shape)
    # print(sample_data['target_img_data'].shape)
    # print(len(sample_data['mod']['str']))

    # logger.add_graph(
    #     model,
    #     [
    #         sample_data['source_img_data'].cuda(),
    #         # [sample_data['mod']['str']],
    #         ['t' for i in range(32)],
    #         sample_data['target_img_data'].cuda()
    #     ]
    # )
